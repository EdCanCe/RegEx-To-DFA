 ...
 * @brief El archivo contiene la clase que permite preprocesar unaexpresión
 * regular para convertirla a su formato prefijo
 *
...
/**
 * @brief Obtiene el operador principal a realizar dentro de un fragmento de una
 * expresión regular.
 *
...
char Preprocessing::getOperator(string regex) {
    ...
    // Verifica si es un 'or' al contar los símbolos que lo representen fuera de
    // paréntesis
    for (int i = 0; i < regex.size(); i++) {
        if (regex[i] == '(')
            ...
            parenthesis--;
        else if (regex[i] == '|' and parenthesis == 0)
            return '|';
    }
    ...
    // En caso de que no sea otro operador, se regresa como operador de
    // concatenar
    return '_';
}

/**
 * @brief Obtiene el índice donde se separa una parte de la expresión de su otra
 * mitad debido al uso de un operador 'or'.
 *
 * @param regex Fragmento de expresión regular a evaluar.
...
int Preprocessing::getSeparation(string regex) {
    ...
    // Encuentra el índice donde se encuentra el punto de separación de la
    // expresión
    for (int i = 0; i < regex.size(); i++) {
        if (regex[i] == '(')
            parenthesis++;
...
vector<string> Preprocessing::getOperands(string regex) {
    ...
    for (int i = 0; i < regex.size(); i++) {
        ...
        // En caso de que el siguiente elemento sea un operador como 'uno o
        // más', se le añade otro caracter más
        if (j < (regex.size() - 1) && in(operators, regex[j + 1])) {
            extra += 1;
        }

        // Se añade el operando dictado por sus paréntesis y operador
...
/**
 * @brief Obtiene la expresión quitando los paréntesis innecesarios a partir de
 * un fragmento de una expresión regular.
 *
 * @param regex Fragmento de expresión regular a evaluar.
...
string Preprocessing::cleanRegex(string regex) {
    ...
    for (int i = 0; i < regex.size(); i++) {
        ...
        // En caso de que el paréntesis no sea el elemento final, significa que
        // SI es necesario el paréntesis
...
/**
 * @brief Obtiene una expresión regular en su formato prefijo a partir de un
...
string Preprocessing::notation(string regex) {
    ...
    // En caso de que tenga más de un operador, pero no sea un 'or', significa
    // que es una concatenación enrealidad
    ...
    switch (operation) {
        case '|': {
            ...
            newRegex += "(" + notation(firstOperand) + ")" + "(" +
                        notation(secondOperand) + ")";
            break;
        }
...
/**
 * @brief Obtiene una expresión regular en su formato prefijo a partir de una
 * expresión regular.
 *
 ...
 * Complejidad asintótica: O(n³)
 */
string Preprocessing::notation() { return notation(originalRegex); }
